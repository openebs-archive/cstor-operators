...
**Note:** Don’t forget to install NFS client packages in all worker nodes.  If NFS client packages are not installed and enabled then it will fail to provision any application which uses the NFS storage class.
