	...
	tests := map[string]struct {
		...
		"event": {
			cstorVolume: &apis.CStorVolume{
				...
				Status: apis.CStorVolumeStatus{
					Phase: apis.CStorVolumePhase(CVStatusHealthy),
				},
			},
			event: &v1.Event{
				ObjectMeta: metav1.ObjectMeta{
				...
				InvolvedObject: v1.ObjectReference{
					Kind:            string(apitypes.CStorVolumeKind),
					APIVersion:      string(apitypes.CStorAPIVersion),
					Name:            "csv-1",
					Namespace:       string(DefaultNameSpace),
					UID:             types.UID("abcd"),
					ResourceVersion: "1111",
				...
				Count:          1,
				Message:        fmt.Sprintf(EventMsgFormatter, "Healthy"),
				Reason:         "Healthy",
				Type:           getEventType(CStorVolumeStatus("Healthy")),
...
func TestGetEventType(t *testing.T) {
	tests := map[string]struct {
		phase     CStorVolumeStatus
		eventType string
	}{
		"Normal event Init":     {phase: CVStatusInit, eventType: v1.EventTypeNormal},
		"Normal event Healthy":  {phase: CVStatusHealthy, eventType: v1.EventTypeNormal},
		"Normal event Degraded": {phase: CVStatusDegraded, eventType: v1.EventTypeNormal},
		"Warning event Error":   {phase: CVStatusError, eventType: v1.EventTypeWarning},
		"Warning event Invalid": {phase: CVStatusInvalid, eventType: v1.EventTypeWarning},
		"Warning event Offline": {phase: CVStatusOffline, eventType: v1.EventTypeWarning},
	}
	for desc, ut := range tests {
		t.Run(desc, func(t *testing.T) {
