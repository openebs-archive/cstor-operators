...
func CreateVolumeReplica(cStorVolumeReplica *cstor.CStorVolumeReplica, fullVolName string, quorum bool) error {
	var cmd []string
	isClone := cStorVolumeReplica.Labels[string(cstor.CloneEnableKEY)] == "true"
	snapName := ""
	...
	if isClone {
		srcVolume := cStorVolumeReplica.Annotations[string(cstor.SourceVolumeKey)]
		snapName = cStorVolumeReplica.Annotations[string(cstor.SnapshotNameKey)]
...
func buildVolumeCloneCommand(cStorVolumeReplica *cstor.CStorVolumeReplica, snapName, fullVolName string) []string {
...
func ZfsToCvrStatusMapper(zfsstatus string, quorum int) string {
	...
	case ZfsStatusHealthy:
		return string(cstor.CVRStatusOnline)
	case ZfsStatusOffline:
		return string(cstor.CVRStatusOffline)
	case ZfsStatusDegraded:
		if quorum == 1 {
			return string(cstor.CVRStatusDegraded)
		}
		return string(cstor.CVRStatusNewReplicaDegraded)
	case ZfsStatusRebuilding:
		if quorum == 1 {
			return string(cstor.CVRStatusRebuilding)
		}
		return string(cstor.CVRStatusReconstructingNewReplica)
	default:
		return string(cstor.CVRStatusError)
	}
}
