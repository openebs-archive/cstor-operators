...
type CStorVolumeClaimLister interface {
	...
	List(selector labels.Selector) (ret []*v1.CStorVolumeClaim, err error)
...
// List lists all CStorVolumeClaims in the indexer.
func (s *cStorVolumeClaimLister) List(selector labels.Selector) (ret []*v1.CStorVolumeClaim, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.CStorVolumeClaim))
	})
	return ret, err
}
...
type CStorVolumeClaimNamespaceLister interface {
	...
	Get(name string) (*v1.CStorVolumeClaim, error)
...
// List lists all CStorVolumeClaims in the indexer for a given namespace.
func (s cStorVolumeClaimNamespaceLister) List(selector labels.Selector) (ret []*v1.CStorVolumeClaim, err error) {
...
// Get retrieves the CStorVolumeClaim from the indexer for a given namespace and name.
func (s cStorVolumeClaimNamespaceLister) Get(name string) (*v1.CStorVolumeClaim, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("cstorvolumeclaim"), name)
	}
	return obj.(*v1.CStorVolumeClaim), nil
}
