...
type CStorBackupLister interface {
	...
	List(selector labels.Selector) (ret []*v1.CStorBackup, err error)
...
// List lists all CStorBackups in the indexer.
func (s *cStorBackupLister) List(selector labels.Selector) (ret []*v1.CStorBackup, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.CStorBackup))
	})
	return ret, err
}
...
type CStorBackupNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1.CStorBackup, error)
...
// List lists all CStorBackups in the indexer for a given namespace.
func (s cStorBackupNamespaceLister) List(selector labels.Selector) (ret []*v1.CStorBackup, err error) {
...
// Get retrieves the CStorBackup from the indexer for a given namespace and name.
func (s cStorBackupNamespaceLister) Get(name string) (*v1.CStorBackup, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("cstorbackup"), name)
	}
	return obj.(*v1.CStorBackup), nil
}
